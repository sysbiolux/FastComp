...
%for i=1:numel(localisedReactions)
%    if isempty(ReactionCompartmentalisationData{i})
%        ReactionCompartmentalisationData{i} = CompIDs{1};
%    end
%end
...
%Attach = sparse(double(diag(TransMetabPresence)));
%Attach(:,sum(Attach,2) == 0) = [];
...
%position=not_empty(IA);
%[r, ~]=ind2sub(size(A_keep), position);
%A_keep2_uni_name=strcat(A_keep2_uni,'(',CompIDs(r)',')');
