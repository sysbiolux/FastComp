function [FCComps,FinalComps,FCTime] = FastCompartPrediction( model , weightedRxns, nonLocSets,...
                                                              localisedReactions, CompIDs, epsilon,...
                                                              noncompmodel, nonLocToOrigMap, ReactionCompartmentalisationData,...
                                                              ComparisonModel,indicator)
...
% weightedRxns              - Reactions that are punished (i.e. transporter reactions)
% nonLocSets                - a Matrix with one Row per non localised reaction and the positions 
%                             of these reactions in the model, non existent reactions are marked with 0 
...
% epsilon                   - Epsilon for fastcore, should be 1
...
% ReactionCompartmentalisationData - The Compartmentalisation data.
% ComparisonModel           - a model for comparison in the preprocessing step
%
...
[new_localised] = compartment_localisation_step( model,CompIDs, localisedReactions,...
                                         ComparisonModel, nonLocSets,ReactionCompartmentalisationData,...
...
while numel(unloc) > 0    
    ...
    ReactionCompartmentalisationData(nonLocToOrigMap) = FinalComps;
    step = step + 1;    
    ...
    if step > stepsize && step > 10
        ...
        [new_localised] = compartment_localisation_step( model,CompIDs,...
            ...
            ReactionCompartmentalisationData, epsilon,Metabolites,AvailableInComp,TransDirections);
...
%fprintf('CalcScores2: The Bounds of R2(c), R2(m) and R2(p) are %f/%f , %f/%f , %f/%f\n',MILP.Model.lb(R2posc),MILP.Model.ub(R2posc),MILP.Model.lb(R2posm),MILP.Model.ub(R2posm),MILP.Model.lb(R2posp),MILP.Model.ub(R2posp))

end

% 
% function models = getCompModels(model, nonLocSets, transporters, localisedReactions, CompIDs)
% 
% olbs = model.lb;
% oubs = model.ub;
% lbs = zeros(size(olbs));
% ubs = zeros(size(oubs));
% trans = logical(zeros(numel(model.rxns),1));
% trans(transporters) = 1;
% locreacs = logical(zeros(numel(model.rxns),1));
% locreacs(localisedReactions) = 1;
% for i=1:numel(CompIDs)    
%     %Restrict the transporters to those leading to the current compartment
%     c_trans = ~(cellfun(@isempty, cellfun(@(x) regexp(x,['^R_TC' upper(CompIDs{i})]), model.rxns(transporters),'UniformOutput',0)));
%     lbs(trans) = olbs(trans).*c_trans;
%     ubs(trans) = oubs(trans).*c_trans;
%     %Restrict the Reactions to those localised to the current compartment.
%     c_loc = ~(cellfun(@isempty, cellfun(@(x) regexp(x,['\(' CompIDs{i} '\)$']), model.rxns(locreacs),'UniformOutput',0)));
%     lbs(locreacs) = lbs(locreacs).*c_loc;
%     ubs(locreacs) = ubs(locreacs).*c_loc;    
%     cnonLocs = nonLocSets;
%     cnonLocs(:,i) = [];
%     lbs(cnonlocs) = 0;
%     ubs(cnonlocs) = 0;
%     model.lb = lbs(:,i);
%     model.ub = ubs(:,i);    
%     models.(CompIDs{i}) = model; 
% end
% end
% 
% 
% function [lp,NP,NL,L,TP,m,n,k,q] = setupLP(model,nonLocSets,localisedReactions, epsilon, transpenalty)
% 
% lp = Cplex('LP');
% % maxflux = 10e4;
% % % set all set bounds to +/-maxflux (at least all that are != 0 to begin with
% % model.ub(find(model.ub > 0)) =  maxflux;
% % model.lb(find(model.lb < 0)) = -maxflux;
% %
% % %convert to integer bounds
% % model.ub = ceil(model.ub);
% % model.lb = floor(model.lb);
% 
% m = numel(model.mets);
% n = numel(model.rxns) ;
% k = numel(nonLocSets);
...
% % size(NL)
% % size(TP)
% % size(diag(model.ub(localisedReactions)+eps))
% % size(diag(model.ub(NP)+eps))
% 
% %Create the A matrix
% A =   [model.S  , sparse(m,q) , sparse(m,t) ; % S*v = 0
%     L , -speye(q,q), sparse(q,t) %  z <= v for all Localised Reactions
...
% 
% 
% lbs = [model.lb;zeros(q,1);zeros(t,1)]; % essentially all the other variables are strictly positive
% ubs = [model.ub;epsilon*ones(q,1);inf * ones(t,1)]; % except for the u's all other helper variables are binary
