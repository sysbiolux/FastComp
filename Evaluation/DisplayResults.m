function [FCResult, FCPureResult,FCFullResult,MOResult,RandomResult] = DisplayResults(Percentages,SampleCount,FileName,Comps,model,excludeC)
...
for prec=1:numel(Percentages)
    ...
    for r = 1:SampleCount
        try
            ...
            for i = 1:length(fieldnames(Predictions))
                ...
                nonexchange = ~ismember(Predictions.(['Replicate' num2str(i)]){6},exchangereactions);
                origdata = Predictions.(['Replicate' num2str(i)]){5}';
                origdata = origdata(nonexchange);
                preddata1 = Predictions.(['Replicate' num2str(i)]){1};
                preddata2 = Predictions.(['Replicate' num2str(i)]){2};
                preddata3 = Predictions.(['Replicate' num2str(i)]){3};
                preddata4 = Predictions.(['Replicate' num2str(i)]){4};
                ...
                [fcfullPlusEMR,fcfullPlusL_Score,fcfullPlusR_Score] = Evaluate(origdata,preddata3,Comps,excludeC);
                ...
                FCPlusResults(end+1,:) = [fcfullPlusEMR,fcfullPlusL_Score,fcfullPlusR_Score];
                MOResults(end+1,:) = [MOEMR,MOL_Score,MOR_Score];
                RandomResults(end+1,:) = [randEMR,randL_Score,randR_Score];
                samples = samples + 1;                
            end
            fprintf('Success\n')
            FCTimes = [FCTimes ; ResultFCPure(:,5)];
            FCPureTimes = [FCPureTimes ; ResultFC(:,5)];
            FCPlusTimes = [FCPlusTimes ; ResultFCPlus(:,5)];
    ...
    fprintf('FCCompP  \t %f5.2 ± %f5.2 \t %f5.2 ± %f5.2\t %f5.2 ± %f5.2\t %f5.2 ± %f5.2\n',mean(FCPlusResults(:,1)),std(FCPlusResults(:,1)),mean(FCPlusResults(:,2)),std(FCPlusResults(:,2)),mean(FCPlusResults(:,3)),std(FCPlusResults(:,3)),mean(FCPlusTimes),std(FCPlusTimes))
    ...
    FCFullResult = [mean(FCPlusResults(:,1)),std(FCPlusResults(:,1)),...
                    mean(FCPlusResults(:,2)),std(FCPlusResults(:,2)),...
                    mean(FCPlusResults(:,3)),std(FCPlusResults(:,3)),...
                    mean(FCPlusTimes),std(FCPlusTimes)];
